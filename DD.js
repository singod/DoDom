var DoDom = (function(doc) {
    var
        ...
        setText = !isUndef(tempEl.innerText) ? function(elem, text) {
            elem.innerText = text;
        } : function(elem, text) {
            elem.textContent = text;
        },
        contains = tempEl.contains ? function(par, chi) {
            return par.contains(chi);
        } : function(par, chi) {
            var res = par.compareDocumentPosition(chi);
            return res === 0 || !!(res & 16);
        },
        overout = {
        ...
        addEvent = doc.addEventListener ? function(el, type, handler) {
            ...
            if (overout[type]) {
                ...
                //     if (contains(root, target) && !contains(root, related)) {
                //         handler.call(root, e);
                //     }
                // } : function (e) {
                //     var target = e.target,
                //         root = this,
                //         related = e.relatedTarget;
                //         console.log(105, target, related,  contains(root, target))
                //     if (contains(root, target) && !contains(root, related)) {
                //         handler.call(root, e);
                //     }
                // };
                return el.addEventListener(overout[type], function(e) {
                    var target = e.target,
                        root = this,
                        related = e.relatedTarget;
                    // 如果包含root包含target(target=root也算包含)，但root不包含related
                    if (contains(root, target) && !contains(root, related)) {
                        handler.call(root, e);
                    }
                });
        ...
        getSiblings = function(elm) {
            var siblings = [],
                first = elm.parentNode.firstChild;
            while (first) {
                // console.log(first)
    ...
    var DoDom = function() {
        ...
        Main.create = function(htmlstr, props) {
            ...
            if (htmlstr[0] === '<') {
                ...
                return new DoDomProxy(temp.firstChild);
            } else {
    ...
    DoDomProxy.prototype = {
        ...
        // 可以传入sel参数对next验证，如验证失败，则有next也会返回null
        next: function(sel) {
            var next = getNext(this.node);
            // 如果有next，且 没有sel或有sel但通过了is验证
            if (next && (!sel || is(next, sel))) {
                return DoDom(next);
            }
            return null;
        },
        prev: function(sel) {
            var prev = getPrev(this.node);
            if (prev && (!sel || is(prev, sel))) {
                return DoDom(prev);
            }
            return null;
        },
        ...
        index: function(elem) {
            var node = this.node;
            var siblings = node.parentNode.chilren,
                sl = siblings.length,
                s = 0,
                sb;
            for (; s < sl;) {
        ...
        is: function(str) {
            var node = this.node;
            return typeof str === 'function' ? str(node) : node.matches(str);
        },
        // 事件代理函数，依赖于is
    ...
    DoDomAll.prototype = {
        ...
        first: function() {
            return DoDom(this.nodes[0]);
        },
        last: function() {
            return DoDom(this.nodes[this.nodes.length - 1]);
        }
    };
